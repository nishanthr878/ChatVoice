...
## Build sequencing (current plan)

1. Layer 2 schema + bare Kafka consumer skeleton (turn in → `turn` row written → echoed back). No LLM, no graph.
2. Trivial single-node Layer 3 pass-through wired into the skeleton (fixed `current_node`, canned response) — proves the concurrency/ordering model end-to-end.
